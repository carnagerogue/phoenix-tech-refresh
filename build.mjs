...
// Root and built entry points must both load motion. Fail instead of silently publishing a static page.
if (!/href="motion\.css(?:\?[^"<>]*)?"/.test(html) || !/src="motion\.js(?:\?[^"<>]*)?"/.test(html)) throw new Error('Source homepage must explicitly load motion.css and motion.js');
