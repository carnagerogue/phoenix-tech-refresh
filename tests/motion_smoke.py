...
with sync_playwright() as p:
    ...
    try:
        ...
        check('Reduced-motion users can choose Play animation', page.locator('.motion-control').is_enabled() and page.locator('.motion-control').inner_text() == 'Play animation')
        pixels=digest()
        page.locator('.motion-control').press('Enter');page.wait_for_timeout(400)
        check('Explicit play overrides reduced motion and changes pixels', status()['reduced'] and status()['running'] and digest()!=pixels)
        check('Playing control offers pause', page.locator('.motion-control').inner_text() == 'Pause animation')
        page.locator('[data-quote]').first.click();page.wait_for_timeout(100)
        check('Explicit playback still suspends behind dialogs', not status()['running'])
        page.locator('#quote-dialog [data-close]').click();page.wait_for_timeout(150)
        check('Explicit playback resumes after dialog closes', status()['running'])
        page.locator('.motion-control').press('Space');page.wait_for_timeout(100)
        a,pixels=status()['frames'],digest();page.wait_for_timeout(300)
        check('Reduced-motion user can pause frames and pixels', not status()['running'] and status()['frames']==a and digest()==pixels)
        page.locator('.motion-control').click();page.wait_for_timeout(150)
        check('Reduced-motion user can play again', status()['running'])
        page.emulate_media(reduced_motion='no-preference');page.wait_for_timeout(100)
        page.emulate_media(reduced_motion='reduce');page.wait_for_timeout(100)
        check('New system preference resets explicit playback', not status()['running'])
        page.emulate_media(reduced_motion='no-preference');page.wait_for_timeout(100)
        ...
        phone=mobile.new_page();load(phone);phone.wait_for_function('window.PTR_MOTION?.version === "2.0.1"')
        phone.on('pageerror', lambda e: errors.append(str(e)))
        if phone.locator('#reject-cookies').is_visible():phone.locator('#reject-cookies').click()
        phone.wait_for_timeout(150)
        check('Initial reduced-motion load stays still on mobile', not phone.evaluate('PTR_MOTION.status.running'))
        check('Mobile play control is enabled', phone.locator('.motion-control').is_enabled())
        phone.locator('.motion-control').tap();phone.wait_for_timeout(300)
        check('Mobile explicit play starts rendering', phone.evaluate('PTR_MOTION.status.running'))
        before=phone.locator('.hero canvas').screenshot()
        phone.wait_for_timeout(400)
        check('Mobile canvas pixels change after play', phone.locator('.hero canvas').screenshot()!=before)
        ...
        check('Reload returns to reduced-motion default', not phone.evaluate('PTR_MOTION.status.running') and phone.locator('.motion-control').inner_text() == 'Play animation')
